...
const fetchAccounts = async () => {
  try {
    const response = await accountApi.getAccounts(1, 10) // 固定分页参数
    // 后端返回 { code: number, data: Account[], msg: string }
  ...
  <div class="account-status">
    ...
    <el-card class="table-card">
      <el-table :data="accounts" style="width: 100%" border stripe highlight-current-row>
        ...
        <el-table-column prop="pwd" label="密码" min-width="100" />
        <el-table-column prop="host" label="host" min-width="100" />
