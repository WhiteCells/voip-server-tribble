...
<template>
  <div class="client-status">
    ...
    <el-card class="table-card">
      <el-table :data="clients" style="width: 100%" border stripe highlight-current-row>
        ...
        <el-table-column label="账号" min-width="200">
          <template #default="{ row }">
            <el-tag v-for="account in row.accounts" :key="account.id" class="account-tag"
              :type="account.status === '已注册' ? 'success' : 'warning'">
              {{ account.name }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 添加/编辑对话框 -->
    <el-dialog v-model="dialogVisible" :title="editingClient ? '编辑客户端' : '添加客户端'" width="500px" destroy-on-close>
      <!-- For debugging: current dialogVisible value: {{ dialogVisible }} -->
